...
//        mylog(getArrayInf($record));
    ...
    if (isset($_POST['altOrder'])) {
        $order_id = $_POST['orderId'];
        $altMode = $_POST['altOrder'];

        if ($altMode == 'onShip' || $altMode == 'altExpress') {
            $express_id = $_POST['value']['express_id'];
            $express_order = $_POST['value']['express_order'];
            pdoUpdate('order_tbl', array('stu' => '2', 'express_id' => $express_id, 'express_order' => $express_order),
                array('id' => $order_id));
            $rid = pdoInsert('order_record_tbl', array('order_id' => $order_id, 'event' => '2', 'operator_id' => $_SESSION['operator_id']));
            if ($rid > 0) {
                include_once '../wechat/serveManager.php';
                $query = pdoQuery('user_express_query_view', null, array('id' => $order_id), ' limit 1');
                $inf = $query->fetch();
                $templateArray = array(
                    ...
                    'remark' => array('value' => '点击下方详情即可查看物流信息')
                );
