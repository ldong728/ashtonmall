...
$(document).on('click', '#add_category', function () {
    $.post('ajax_request.php', {addNewCategory: 1, g_id: g_id}, function (data) {
            $('#button_area').before( '<div class="detail-block">规格：<input class="detail-input category textbox textbox_295" type="text" id="' + data + '"value="规格' +data + '"/>' +
...
function getGInf() {
    ...
    $.post("ajax_request.php", {get_g_inf:1,g_id: g_id}, function (data) {
        ...
        if(0==inf.goodsInf.situation){
            var stub='<a class="link_btn" href="consle.php?goodsSituation=1&g_id='+g_id+'">上架</a>'
        }else{
            var stub='<a class="link_btn" href="consle.php?goodsSituation=0&g_id='+g_id+'">下架</a>'
        }
        ...
        if(null!=inf.detail) {
            ...
            $('#goods_detail').append('<div class="detail-block" id="button_area"><a class="divButton link_btn" id="add-button"><span id="add_category">添加规格</span></a></div>');
        }
        if (null != inf.img) {
        ...
        $.each(inf.parm,function(k,v){
            ...
            $.each(v,function(subk,subv){
                var scon='<tr><td>'+subv.name+'</td><td><textarea name="'+subv.col+'" cols="50" rows="2">'+subv.value+'</textarea></td></tr>'
                con+=scon;
            });
            con+='</table>'
            precon+=con
        });
        $('.parm-set').append(precon+'<button class="link_btn">提交参数修改</button></form>');
